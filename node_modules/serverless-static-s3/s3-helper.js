...
const uploadFilesToS3Bucket = function(bucket, directory, original, replace) {
	replaceAndUpload(bucket, directory, directory, original, replace);
};

const replaceAndUpload = function(bucket, origin, dir, original, replace) {
	fs.readdir(dir, function(err, files) {
	    ...
	    fs.stat(toPath, function( error, stat ) {
	      ...
	      if(stat.isFile()) {
	      	...
	      	replaceAndUpload(bucket, origin, toPath, original, replace)
	      }
	    });
		});
	});
